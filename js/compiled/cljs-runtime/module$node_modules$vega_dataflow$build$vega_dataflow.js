...
this.changeset().insert(data))},parse:function(data,format){const locale=this.locale();return vegaLoader.read(data,format,locale.timeParse,locale.utcParse)},preload:async function(target,url,format){const pending=this._pending||loadPending(this);pending.requests+=1;url=await this.request(url,format);this.pulse(target,this.changeset().remove(vegaUtil.truthy).insert(url.data||[]));pending.done();return url},request:async function(url,format){let status=0,data;try{data=await this.loader().load(url,{context:"dataflow",
response:vegaLoader.responseType(format&&format.type)});try{data=this.parse(data,format)}catch(err){status=-2,this.warn("Data ingestion failed",url,err)}}catch(err){status=-1,this.warn("Loading failed",url,err)}return{data,status}},events:function(source,type,filter,apply){const df=this,s=stream$jscomp$0(filter,apply);filter=function(e){e.dataflow=df;try{s.receive(e)}catch(error){df.error(error)}finally{df.run()}};source="string"===typeof source&&"undefined"!==typeof document?document.querySelectorAll(source):
vegaUtil.array(source);apply=source.length;for(let i=0;i<apply;++i)source[i].addEventListener(type,filter);return s},on:function(source,target,update,params,options){(source instanceof Operator?onOperator:onStream)(this,source,target,update,params,options);return this},evaluate:async function(encode,prerun,postrun){const df=this,async=[];if(df._pulse)return reentrant(df);df._pending&&await df._pending;prerun&&await asyncCallback(df,prerun);if(!df._touched.length)return df.debug("Dataflow invoked, but nothing to do."),
df;prerun=++df._clock;df._pulse=new Pulse(df,prerun,encode);df._touched.forEach(op=>df._enqueue(op,!0));df._touched=UniqueList(vegaUtil.id);let count=0;let next,error;try{for(;0<df._heap.size();){var op$jscomp$0=df._heap.pop();op$jscomp$0.rank!==op$jscomp$0.qrank?df._enqueue(op$jscomp$0,!0):(next=op$jscomp$0.run(df._getPulse(op$jscomp$0,encode)),next.then?next=await next:next.async&&(async.push(next.async),next=StopPropagation),next!==StopPropagation&&op$jscomp$0._targets&&op$jscomp$0._targets.forEach(op=>
df._enqueue(op)),++count)}}catch(err){df._heap.clear(),error=err}df._input={};df._pulse=null;df.debug(`Pulse ${prerun}: ${count} operators`);error&&(df._postrun=[],df.error(error));if(df._postrun.length)for(encode=df._postrun.sort((a,b)=>b.priority-a.priority),df._postrun=[],op$jscomp$0=0;op$jscomp$0<encode.length;++op$jscomp$0)await asyncCallback(df,encode[op$jscomp$0].callback);postrun&&await asyncCallback(df,postrun);async.length&&Promise.all(async).then(cb=>df.runAsync(null,()=>{cb.forEach(f=>
{try{f(df)}catch(err){df.error(err)}})}));return df},run:function(encode,prerun,postrun){return this._pulse?reentrant(this):(this.evaluate(encode,prerun,postrun),this)},runAsync:async function(encode,prerun,postrun){for(;this._running;)await this._running;const clear=()=>this._running=null;(this._running=this.evaluate(encode,prerun,postrun)).then(clear,clear);return this._running},runAfter:function(callback,enqueue,priority){if(this._pulse||enqueue)this._postrun.push({priority:priority||0,callback});
else try{callback(this)}catch(err){this.error(err)}},_enqueue:function(op,force){const q=op.stamp<this._clock;q&&(op.stamp=this._clock);if(q||force)op.qrank=op.rank,this._heap.push(op)},_getPulse:function(op,encode){const s=op.source,stamp=this._clock;return s&&vegaUtil.isArray(s)?new MultiPulse(this,stamp,s.map(_=>_.pulse),encode):this._input[op.id]||singlePulse(this._pulse,s&&s.pulse)}};vegaUtil.inherits(Transform,Operator,{run(pulse){if(pulse.stamp<this.stamp)return pulse.StopPropagation;let rv;
this.skip()?this.skip(!1):rv=this.evaluate(pulse);rv=rv||pulse;rv.then?rv=rv.then(_=>this.pulse=_):rv!==pulse.StopPropagation&&(this.pulse=rv);return rv},evaluate(pulse){const params=this.marshall(pulse.stamp);pulse=this.transform(params,pulse);params.clear();return pulse},transform(){}});const transforms={};exports.Dataflow=Dataflow;exports.EventStream=EventStream;exports.MultiPulse=MultiPulse;exports.Operator=Operator;exports.Parameters=Parameters;exports.Pulse=Pulse;exports.Transform=Transform;
exports.UniqueList=UniqueList;exports.asyncCallback=asyncCallback;exports.changeset=changeset;exports.definition=function(type){return(type=transform(type))&&type.Definition||null};exports.derive=function(t){return rederive(t,ingest$1({}))};exports.ingest=ingest$1;exports.isChangeSet=isChangeSet;exports.isTuple=function(t){return!(!t||!t[TUPLE_ID_KEY])};exports.rederive=rederive;exports.replace=function(t,d){d[TUPLE_ID_KEY]=t[TUPLE_ID_KEY];return d};exports.stableCompare=function(cmp,f){return cmp?
f?(a,b)=>cmp(a,b)||f(a)[TUPLE_ID_KEY]-f(b)[TUPLE_ID_KEY]:(a,b)=>cmp(a,b)||a[TUPLE_ID_KEY]-b[TUPLE_ID_KEY]:null};exports.transform=transform;exports.transforms=transforms;exports.tupleid=function(t){return t[TUPLE_ID_KEY]};Object.defineProperty(exports,"__esModule",{value:!0})})}
