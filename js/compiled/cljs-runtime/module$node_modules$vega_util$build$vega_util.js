...
opt=function(_){let s=""+get[0](_),i=0;for(;++i<len;)s+="|"+get[i](_);return s}}else opt=function(){return""};return accessor(opt,fields,"key")};exports.lerp=function(array,frac){const lo=array[0];array=peek(array);return(frac=+frac)?1===frac?array:lo+frac*(array-lo):lo};exports.logger=function(_$jscomp$0,method){let level=_$jscomp$0||0;return{level(_){return arguments.length?(level=+_,this):level},error(){1<=level&&log$1(method||"error","ERROR",arguments);return this},warn(){2<=level&&log$1(method||
"warn","WARN",arguments);return this},info(){3<=level&&log$1(method||"log","INFO",arguments);return this},debug(){4<=level&&log$1(method||"log","DEBUG",arguments);return this}}};exports.lruCache=function(maxsize){maxsize=+maxsize||1E4;let curr,prev,size;const clear=()=>{curr={};prev={};size=0},update=(key,value)=>{++size>maxsize&&(prev=curr,curr={},size=1);return curr[key]=value};clear();return{clear,has:key=>has(curr,key)||has(prev,key),get:key=>has(curr,key)?curr[key]:has(prev,key)?update(key,prev[key]):
void 0,set:(key,value)=>has(curr,key)?curr[key]=value:update(key,value)}};exports.merge=function(compare,array0,array1,output){const n0=array0.length,n1=array1.length;if(!n1)return array0;if(!n0)return array1;output=output||new array0.constructor(n0+n1);let i0=0,i1=0,i=0;for(;i0<n0&&i1<n1;++i)output[i]=0<compare(array0[i0],array1[i1])?array1[i1++]:array0[i0++];for(;i0<n0;++i0,++i)output[i]=array0[i0];for(;i1<n1;++i1,++i)output[i]=array1[i1];return output};exports.mergeConfig=function(...configs){return configs.reduce((out,
source)=>{for(const key in source)"signals"===key?out.signals=mergeNamed(out.signals,source.signals):writeConfig(out,key,source[key],"legend"===key?{layout:1}:"style"===key?!0:null);return out},{})};exports.one=one;exports.pad=function(str,length,padchar,align){padchar=padchar||" ";str+="";length-=str.length;return 0>=length?str:"left"===align?repeat(padchar,length)+str:"center"===align?repeat(padchar,~~(length/2))+str+repeat(padchar,Math.ceil(length/2)):str+repeat(padchar,length)};exports.panLinear=
