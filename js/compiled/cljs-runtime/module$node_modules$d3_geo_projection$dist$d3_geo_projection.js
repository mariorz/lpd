...
polygon,ring});for(var i=0,n=ring.length;i<n;++i){var point=ring[i],x=point[0],y=point[1];if(-179.9999>=x||179.9999<=x||-89.9999>=y||89.9999<=y){ring[i]=clampPoint(point);for(point=i+1;point<n;++point){var pointk=ring[point];x=pointk[0];pointk=pointk[1];if(-179.9999<x&&179.9999>x&&-89.9999<pointk&&89.9999>pointk)break}if(point!==i+1){i?(i={index:-1,polygon,ring:ring.slice(0,i+1)},i.ring[i.ring.length-1]=normalizePoint(y),fragments[fragments.length-1]=i):fragments.pop();if(point>=n)break;fragments.push({index:-1,
polygon,ring:ring=ring.slice(point-1)});ring[0]=normalizePoint(ring[0][1]);i=-1;n=ring.length}}}}}function stitchFragments(fragments){var i,n=fragments.length,fragmentByStart={},fragmentByEnd={};for(i=0;i<n;++i){var fragment=fragments[i];var start=fragment.ring[0];var end=fragment.ring[fragment.ring.length-1];start[0]===end[0]&&start[1]===end[1]?(fragment.polygon.push(fragment.ring),fragments[i]=null):(fragment.index=i,fragmentByStart[start]=fragmentByEnd[end]=fragment)}for(i=0;i<n;++i)if(fragment=
fragments[i]){start=fragment.ring[0];end=fragment.ring[fragment.ring.length-1];var startFragment=fragmentByEnd[start];var endFragment=fragmentByStart[end];delete fragmentByStart[start];delete fragmentByEnd[end];start[0]===end[0]&&start[1]===end[1]?fragment.polygon.push(fragment.ring):startFragment?(delete fragmentByEnd[start],delete fragmentByStart[startFragment.ring[0]],startFragment.ring.pop(),fragments[startFragment.index]=null,fragment={index:-1,polygon:startFragment.polygon,ring:startFragment.ring.concat(fragment.ring)},
startFragment===endFragment?fragment.polygon.push(fragment.ring):(fragment.index=n++,fragments.push(fragmentByStart[fragment.ring[0]]=fragmentByEnd[fragment.ring[fragment.ring.length-1]]=fragment))):endFragment?(delete fragmentByStart[end],delete fragmentByEnd[endFragment.ring[endFragment.ring.length-1]],fragment.ring.pop(),fragment={index:n++,polygon:endFragment.polygon,ring:fragment.ring.concat(endFragment.ring)},fragments[endFragment.index]=null,fragments.push(fragmentByStart[fragment.ring[0]]=
...
0]).scale(1).rotate(0<face[1]?[-face[0],0]:[180-face[0],180])};var faces=octahedron$1.map(function(face){return{face,project:faceProjection(face)}});[-1,0,0,1,0,1,4,5].forEach(function(d,i){(d=faces[d])&&(d.children||(d.children=[])).push(faces[i])});return polyhedral(faces[0],function(lambda,phi){return faces[lambda<-pi$jscomp$0/2?0>phi?6:4:0>lambda?0>phi?2:0:lambda<pi$jscomp$0/2?0>phi?3:1:0>phi?7:5]}).angle(-30).scale(121.906).center([0,48.5904])};exports.geoPolyhedralWaterman=function(faceProjection){faceProjection=
...
i+1)%(2*n)])))});var faces=w5.map(function(face){return{project:faceProjection(face),face}});parents.forEach(function(d,i){(d=faces[d])&&(d.children||(d.children=[])).push(faces[i])});return polyhedral(faces[0],function(lambda,phi){var cosphi=cos(phi);cosphi=[cosphi*cos(lambda),cosphi*sin(lambda),sin(phi)];lambda=lambda<-pi$jscomp$0/2?0>phi?6:4:0>lambda?0>phi?2:0:lambda<pi$jscomp$0/2?0>phi?3:1:0>phi?7:5;phi=cornerNormals[lambda];return faces[0>dot(phi[0],cosphi)?8+3*lambda:0>dot(phi[1],cosphi)?3*
lambda+9:0>dot(phi[2],cosphi)?3*lambda+10:lambda]}).angle(-30).scale(110.625).center([0,45])};exports.geoProject=function(object,projection){projection=projection.stream;if(!projection)throw Error("invalid projection");switch(object&&object.type){case "Feature":var project=projectFeature;break;case "FeatureCollection":project=projectFeatureCollection;break;default:project=projectGeometry}return project(object,projection)};exports.geoGringortenQuincuncial=function(){return quincuncial(gringortenRaw).scale(176.423)};
