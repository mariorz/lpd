...
valueof)for(var value$jscomp$0 of values)null!=value$jscomp$0&&(value$jscomp$0=+value$jscomp$0)>=value$jscomp$0&&++count;else{value$jscomp$0=-1;for(let value of values)null!=(value=valueof(value,++value$jscomp$0,values))&&(value=+value)>=value&&++count}return count}function length$1(array){return array.length|0}function empty(length){return!(0<length)}function arrayify(values){return"object"!==typeof values||"length"in values?values:Array.from(values)}function reducer$jscomp$0(reduce){return values=>
reduce(...values)}function variance(values,valueof){let count=0,delta,mean=0,sum=0;if(void 0===valueof)for(var value$jscomp$0 of values)null!=value$jscomp$0&&(value$jscomp$0=+value$jscomp$0)>=value$jscomp$0&&(delta=value$jscomp$0-mean,mean+=delta/++count,sum+=delta*(value$jscomp$0-mean));else{value$jscomp$0=-1;for(let value of values)null!=(value=valueof(value,++value$jscomp$0,values))&&(value=+value)>=value&&(delta=value-mean,mean+=delta/++count,sum+=delta*(value-mean))}if(1<count)return sum/(count-
1)}function deviation(values,valueof){return(values=variance(values,valueof))?Math.sqrt(values):values}function extent(values,valueof){let min,max;if(void 0===valueof)for(var value$jscomp$0 of values)null!=value$jscomp$0&&(void 0===min?value$jscomp$0>=value$jscomp$0&&(min=max=value$jscomp$0):(min>value$jscomp$0&&(min=value$jscomp$0),max<value$jscomp$0&&(max=value$jscomp$0)));else{value$jscomp$0=-1;for(let value of values)null!=(value=valueof(value,++value$jscomp$0,values))&&(void 0===min?value>=value&&
(min=max=value):(min>value&&(min=value),max<value&&(max=value)))}return[min,max]}function intern_get({_intern,_key},value){_key=_key(value);return _intern.has(_key)?_intern.get(_key):value}function intern_set({_intern,_key},value){_key=_key(value);if(_intern.has(_key))return _intern.get(_key);_intern.set(_key,value);return value}function intern_delete({_intern,_key},value){_key=_key(value);_intern.has(_key)&&(value=_intern.get(value),_intern.delete(_key));return value}function keyof(value){return null!==
value&&"object"===typeof value?value.valueOf():value}function identity(x){return x}function group$jscomp$0(values,...keys){return nest(values,identity,identity,keys)}function rollup(values,reduce,...keys){return nest(values,identity,reduce,keys)}function unique(values){if(1!==values.length)throw Error("duplicate key");return values[0]}function nest(values$jscomp$1,map,reduce,keys){return function regroup(values$jscomp$0,i){if(i>=keys.length)return reduce(values$jscomp$0);const groups=new InternMap,
keyof=keys[i++];let index=-1;for(const value of values$jscomp$0){const key=keyof(value,++index,values$jscomp$0),group=groups.get(key);group?group.push(value):groups.set(key,[value])}for(const [key,values]of groups)groups.set(key,regroup(values,i));return map(groups)}(values$jscomp$1,0)}function permute(source,keys){return Array.from(keys,key=>source[key])}function sort(values,...F){if("function"!==typeof values[Symbol.iterator])throw new TypeError("values is not iterable");values=Array.from(values);
let [f$jscomp$0=ascending]=F;if(1===f$jscomp$0.length||1<F.length){const index=Uint32Array.from(values,(d,i)=>i);1<F.length?(F=F.map(f=>values.map(f)),index.sort((i,j)=>{for(const f of F){const c=ascending(f[i],f[j]);if(c)return c}})):(f$jscomp$0=values.map(f$jscomp$0),index.sort((i,j)=>ascending(f$jscomp$0[i],f$jscomp$0[j])));return permute(values,index)}return values.sort(f$jscomp$0)}function constant(x){return function(){return x}}function ticks(start,stop,count){var reverse,i=-1,step;stop=+stop;
start=+start;count=+count;if(start===stop&&0<count)return[start];if(reverse=stop<start){var n=start;start=stop;stop=n}if(0===(step=tickIncrement(start,stop,count))||!isFinite(step))return[];if(0<step)for(count=Math.round(start/step),n=Math.round(stop/step),count*step<start&&++count,n*step>stop&&--n,start=Array(n=n-count+1);++i<n;)start[i]=(count+i)*step;else for(step=-step,count=Math.round(start*step),n=Math.round(stop*step),count/step<start&&++count,n/step>stop&&--n,start=Array(n=n-count+1);++i<
n;)start[i]=(count+i)/step;reverse&&start.reverse();return start}function tickIncrement(start,stop,count){stop=(stop-start)/Math.max(0,count);start=Math.floor(Math.log(stop)/Math.LN10);stop/=Math.pow(10,start);return 0<=start?(stop>=e10?10:stop>=e5?5:stop>=e2?2:1)*Math.pow(10,start):-Math.pow(10,-start)/(stop>=e10?10:stop>=e5?5:stop>=e2?2:1)}function nice(start,stop,count){let prestep;for(;;){const step=tickIncrement(start,stop,count);if(step===prestep||0===step||!isFinite(step))return[start,stop];
0<step?(start=Math.floor(start/step)*step,stop=Math.ceil(stop/step)*step):0>step&&(start=Math.ceil(start*step)/step,stop=Math.floor(stop*step)/step);prestep=step}}function sturges(values){return Math.ceil(Math.log(count$jscomp$0(values))/Math.LN2)+1}function bin(){function histogram(data){Array.isArray(data)||(data=Array.from(data));var i,n=data.length,values=Array(n);for(i=0;i<n;++i)values[i]=value(data[i],i,data);var xz=domain(values),x0=xz[0];xz=xz[1];var tz=threshold(values,x0,xz);if(!Array.isArray(tz)){i=
xz;var tn=+tz;domain===extent&&([x0,xz]=nice(x0,xz,tn));tz=ticks(x0,xz,tn);tz[tz.length-1]>=xz&&(i>=xz&&domain===extent?(i=tickIncrement(x0,xz,tn),isFinite(i)&&(0<i?xz=(Math.floor(xz/i)+1)*i:0>i&&(xz=(Math.ceil(xz*-i)+1)/-i))):tz.pop())}for(tn=tz.length;tz[0]<=x0;)tz.shift(),--tn;for(;tz[tn-1]>xz;)tz.pop(),--tn;var bins=Array(tn+1);for(i=0;i<=tn;++i){var x=bins[i]=[];x.x0=0<i?tz[i-1]:x0;x.x1=i<tn?tz[i]:xz}for(i=0;i<n;++i)x=values[i],x0<=x&&x<=xz&&bins[bisectRight(tz,x,0,tn)].push(data[i]);return bins}
...
valueof){let max;if(void 0===valueof)for(var value$jscomp$0 of values)null!=value$jscomp$0&&(max<value$jscomp$0||void 0===max&&value$jscomp$0>=value$jscomp$0)&&(max=value$jscomp$0);else{value$jscomp$0=-1;for(let value of values)null!=(value=valueof(value,++value$jscomp$0,values))&&(max<value||void 0===max&&value>=value)&&(max=value)}return max}function min$jscomp$0(values,valueof){let min;if(void 0===valueof)for(var value$jscomp$0 of values)null!=value$jscomp$0&&(min>value$jscomp$0||void 0===min&&
value$jscomp$0>=value$jscomp$0)&&(min=value$jscomp$0);else{value$jscomp$0=-1;for(let value of values)null!=(value=valueof(value,++value$jscomp$0,values))&&(min>value||void 0===min&&value>=value)&&(min=value)}return min}function quickselect(array,k,left=0,right=array.length-1,compare=ascending){for(;right>left;){if(600<right-left){var n=right-left+1,m=k-left+1,z=Math.log(n),s=.5*Math.exp(2*z/3);z=.5*Math.sqrt(z*s*(n-s)/n)*(0>m-n/2?-1:1);quickselect(array,k,Math.max(left,Math.floor(k-m*s/n+z)),Math.min(right,
Math.floor(k+(n-m)*s/n+z)),compare)}n=array[k];m=left;s=right;swap(array,left,k);for(0<compare(array[right],n)&&swap(array,left,right);m<s;){swap(array,m,s);++m;for(--s;0>compare(array[m],n);)++m;for(;0<compare(array[s],n);)--s}0===compare(array[left],n)?swap(array,left,s):(++s,swap(array,s,right));s<=k&&(left=s+1);k<=s&&(right=s-1)}return array}function swap(array,i,j){const t=array[i];array[i]=array[j];array[j]=t}function quantile(values,p,valueof){values=Float64Array.from(numbers(values,valueof));
if(valueof=values.length){if(0>=(p=+p)||2>valueof)return min$jscomp$0(values);if(1<=p)return max$jscomp$0(values);p*=valueof-1;valueof=Math.floor(p);var value0=max$jscomp$0(quickselect(values,valueof).subarray(0,valueof+1));values=min$jscomp$0(values.subarray(valueof+1));return value0+(values-value0)*(p-valueof)}}function maxIndex(values,valueof){let max,maxIndex=-1,index=-1;if(void 0===valueof)for(const value of values)++index,null!=value&&(max<value||void 0===max&&value>=value)&&(max=value,maxIndex=
index);else for(let value of values)null!=(value=valueof(value,++index,values))&&(max<value||void 0===max&&value>=value)&&(max=value,maxIndex=index);return maxIndex}function*flatten(arrays){for(const array of arrays)yield*array}function minIndex(values,valueof){let min,minIndex=-1,index=-1;if(void 0===valueof)for(const value of values)++index,null!=value&&(min>value||void 0===min&&value>=value)&&(min=value,minIndex=index);else for(let value of values)null!=(value=valueof(value,++index,values))&&(min>
value||void 0===min&&value>=value)&&(min=value,minIndex=index);return minIndex}function pair(a,b){return[a,b]}function leastIndex(values,compare=ascending){if(1===compare.length)return minIndex(values,compare);let minValue,min=-1,index=-1;for(const value of values)if(++index,0>min?0===compare(value,value):0>compare(value,minValue))minValue=value,min=index;return min}function shuffler(random){return function(array,i0=0,i1=array.length){for(i1-=i0=+i0;i1;){const i=random()*i1--|0,t=array[i1+i0];array[i1+
i0]=array[i+i0];array[i+i0]=t}return array}}function transpose$jscomp$0(matrix){if(!(n=matrix.length))return[];for(var i=-1,m=min$jscomp$0(matrix,length$jscomp$0),transpose=Array(m);++i<m;)for(var j=-1,n,row=transpose[i]=Array(n);++j<n;)row[j]=matrix[j][i];return transpose}function length$jscomp$0(d){return d.length}function set$jscomp$0(values){return values instanceof Set?values:new Set(values)}function superset(values,other){values=values[Symbol.iterator]();const set=new Set;for(const o of other){if(set.has(o))continue;
...
Array.from,identity,keys)};exports.histogram=bin;exports.index=function(values,...keys){return nest(values,identity,unique,keys)};exports.indexes=function(values,...keys){return nest(values,Array.from,unique,keys)};exports.intersection=function(values,...others){values=new Set(values);others=others.map(set$jscomp$0);a:for(const value of values)for(const other of others)if(!other.has(value)){values.delete(value);continue a}return values};exports.least=function(values,compare=ascending){let min,defined=
