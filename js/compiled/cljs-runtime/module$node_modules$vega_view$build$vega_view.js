...
function resizeRenderer(view){var origin=offset(view),w=width$jscomp$0(view),h=height$jscomp$0(view);view._renderer.background(view.background());view._renderer.resize(w,h,origin);view._handler.origin(origin);view._resizeListeners.forEach(handler=>{try{handler(w,h)}catch(error){view.error(error)}})}function eventExtend(view,event,item){var r=view._renderer;r=r&&r.canvas();if(r){var translate=offset(view);var p=event.changedTouches?event.changedTouches[0]:event;p=vegaScenegraph.point(p,r);p[0]-=translate[0];
p[1]-=translate[1]}event.dataflow=view;event.item=item;event.vega=extension(view,item,p);return event}function extension(view,item$jscomp$0,point){function group(name){var g=itemGroup,i;if(name)for(i=item$jscomp$0;i;i=i.mark.group)if(i.mark.name===name){g=i;break}return g&&g.mark&&g.mark.interactive?g:{}}function xy(item){if(!item)return point;vegaUtil.isString(item)&&(item=group(item));const p=point.slice();for(;item;)p[0]-=item.x||0,p[1]-=item.y||0,item=item.mark&&item.mark.group;return p}const itemGroup=
item$jscomp$0?"group"===item$jscomp$0.mark.marktype?item$jscomp$0:item$jscomp$0.mark.group:null;return{view:vegaUtil.constant(view),item:vegaUtil.constant(item$jscomp$0||{}),group,xy,x:item=>xy(item)[0],y:item=>xy(item)[1]}}function initializeEventConfig(config){config=vegaUtil.extend({defaults:{}},config);const unpack=(obj,keys)=>{keys.forEach(k=>{vegaUtil.isArray(obj[k])&&(obj[k]=vegaUtil.toSet(obj[k]))})};unpack(config.defaults,["prevent","allow"]);unpack(config,["view","window","selector"]);return config}
...
name)}}function element(tag,attr,text){tag=document.createElement(tag);for(const key in attr)tag.setAttribute(key,attr[key]);null!=text&&(tag.textContent=text);return tag}function bind$jscomp$0(view,el,binding){if(el){var param=binding.param,bind=binding.state;bind||(bind=binding.state={elements:null,active:!1,set:null,update:value=>{value!=view.signal(param.signal)&&view.runAsync(null,()=>{bind.source=!0;view.signal(param.signal,value)})}},param.debounce&&(bind.update=vegaUtil.debounce(param.debounce,
bind.update)));(null==param.input&&param.element?target:generate)(bind,el,param,view);bind.active||(view.on(view._signals[param.signal],null,()=>{bind.source?bind.source=!1:bind.set(view.signal(param.signal))}),bind.active=!0);return bind}}function target(bind,node,param,view){const type=param.event||"input",handler=()=>bind.update(node.value);view.signal(param.signal,node.value);node.addEventListener(type,handler);trackEventListener(view,node,type,handler);bind.set=value=>{node.value=value;node.dispatchEvent(event$jscomp$0(type))}}
function event$jscomp$0(type){return"undefined"!==typeof Event?new Event(type):{type}}function generate(bind,el,param,view){view=view.signal(param.signal);const div=element("div",{"class":"vega-bind"}),wrapper="radio"===param.input?div:div.appendChild(element("label"));wrapper.appendChild(element("span",{"class":"vega-bind-name"},param.name||param.signal));el.appendChild(div);el=form;switch(param.input){case "checkbox":el=checkbox;break;case "select":el=select;break;case "radio":el=radio;break;case "range":el=
range}el(bind,wrapper,param,view)}function form(bind,el,param,value$jscomp$0){const node=element("input");for(const key in param)"signal"!==key&&"element"!==key&&node.setAttribute("input"===key?"type":key,param[key]);node.setAttribute("name",param.signal);node.value=value$jscomp$0;el.appendChild(node);node.addEventListener("input",()=>bind.update(node.value));bind.elements=[node];bind.set=value=>node.value=value}function checkbox(bind,el,param,value$jscomp$0){param={type:"checkbox",name:param.signal};
value$jscomp$0&&(param.checked=!0);const node=element("input",param);el.appendChild(node);node.addEventListener("change",()=>bind.update(node.checked));bind.elements=[node];bind.set=value=>node.checked=!!value||null}function select(bind,el,param,value$jscomp$0){const node=element("select",{name:param.signal}),labels=param.labels||[];param.options.forEach((option,i)=>{const attr={value:option};valuesEqual(option,value$jscomp$0)&&(attr.selected=!0);node.appendChild(element("option",attr,(labels[i]||
option)+""))});el.appendChild(node);node.addEventListener("change",()=>{bind.update(param.options[node.selectedIndex])});bind.elements=[node];bind.set=value=>{for(let i=0,n=param.options.length;i<n;++i)if(valuesEqual(param.options[i],value)){node.selectedIndex=i;break}}}function radio(bind,el,param,value$jscomp$0){const group=element("span",{"class":"vega-bind-radio"}),labels=param.labels||[];el.appendChild(group);bind.elements=param.options.map((option,i)=>{var attr={type:"radio",name:param.signal,
value:option};valuesEqual(option,value$jscomp$0)&&(attr.checked=!0);attr=element("input",attr);attr.addEventListener("change",()=>bind.update(option));i=element("label",{},(labels[i]||option)+"");i.prepend(attr);group.appendChild(i);return attr});bind.set=value=>{const nodes=bind.elements,n=nodes.length;for(let i=0;i<n;++i)valuesEqual(nodes[i].value,value)&&(nodes[i].checked=!0)}}function range(bind,el,param,value$jscomp$0){value$jscomp$0=void 0!==value$jscomp$0?value$jscomp$0:(+param.max+ +param.min)/
2;const max=null!=param.max?param.max:Math.max(100,+value$jscomp$0)||100,min=param.min||Math.min(0,max,+value$jscomp$0)||0,step=param.step||d3Array.tickStep(min,max,100),node=element("input",{type:"range",name:param.signal,min,max,step});node.value=value$jscomp$0;const span=element("span",{},+value$jscomp$0);el.appendChild(node);el.appendChild(span);el=()=>{span.textContent=node.value;bind.update(+node.value)};node.addEventListener("input",el);node.addEventListener("change",el);bind.elements=[node];
bind.set=value=>{node.value=value;span.textContent=value}}function valuesEqual(a,b){return a===b||a+""===b+""}function initializeRenderer(view,r,el,constructor,scaleFactor,opt){r=r||new constructor(view.loader());return r.initialize(el,width$jscomp$0(view),height$jscomp$0(view),offset(view),scaleFactor,opt).background(view.background())}function trap(view,fn){return fn?function(){try{fn.apply(this,arguments)}catch(error){view.error(error)}}:null}function initializeHandler(view,prevHandler,el,constructor){const handler=
(new constructor(view.loader(),trap(view,view.tooltip()))).scene(view.scenegraph().root).initialize(el,offset(view),view);prevHandler&&prevHandler.handlers().forEach(h=>{handler.on(h.type,h.handler)});return handler}function lookup(view,el,clear){if("string"===typeof el)if("undefined"!==typeof document){if(el=document.querySelector(el),!el)return view.error("Signal bind element not found: "+el),null}else return view.error("DOM document instance not found."),null;if(el&&clear)try{el.innerHTML=""}catch(e){el=
null,view.error(e)}return el}function padding$jscomp$0(_){vegaUtil.isObject(_)?_={top:+_.top||0,bottom:+_.bottom||0,left:+_.left||0,right:+_.right||0}:(_=+_||0,_={top:_,bottom:_,left:_,right:_});return _}async function renderHeadless(view,type,scaleFactor,opt){var module=vegaScenegraph.renderModule(type);(module=module&&module.headless)||vegaUtil.error("Unrecognized renderer type: "+type);await view.runAsync();return initializeRenderer(view,null,null,module,scaleFactor,opt).renderAsync(view._scenegraph.root)}
function runtime(view,spec,expr){return vegaRuntime.context(view,vegaDataflow.transforms,vegaFunctions.functionContext,expr).parse(spec)}function viewWidth$jscomp$0(view,width){var a=view.autosize();view=view.padding();return width-(a&&"padding"===a.contains?view.left+view.right:0)}function viewHeight$jscomp$0(view,height){var a=view.autosize();view=view.padding();return height-(a&&"padding"===a.contains?view.top+view.bottom:0)}function initializeResize(view){function resetSize(){view._autosize=view._resize=
1}var s=view._signals,w=s.width,h=s.height;s=s.padding;view._resizeWidth=view.add(null,_=>{view._width=_.size;view._viewWidth=viewWidth$jscomp$0(view,_.size);resetSize()},{size:w});view._resizeHeight=view.add(null,_=>{view._height=_.size;view._viewHeight=viewHeight$jscomp$0(view,_.size);resetSize()},{size:h});const resizePadding=view.add(null,resetSize,{pad:s});view._resizeWidth.rank=w.rank+1;view._resizeHeight.rank=h.rank+1;resizePadding.rank=s.rank+1}function dataTest(name,data){return data.modified&&
vegaUtil.isArray(data.input.value)&&name.indexOf("_:vega:_")}function signalTest(name,op){return!("parent"===name||op instanceof vegaDataflow.transforms.proxy)}function defaultTooltip(handler,event,item,value){(handler=handler.element())&&handler.setAttribute("title",formatTooltip(value))}function formatTooltip(value){return null==value?"":vegaUtil.isArray(value)?"["+value.map(formatValue).join(", ")+"]":vegaUtil.isObject(value)&&!vegaUtil.isDate(value)?formatObject(value):value+""}function formatObject(obj){return Object.keys(obj).map(key=>
{const v=obj[key];return key+": "+(vegaUtil.isArray(v)?"["+v.map(formatValue).join(", ")+"]":formatValue(v))}).join("\n")}function formatValue(value){return vegaUtil.isArray(value)?"[…]":vegaUtil.isObject(value)&&!vegaUtil.isDate(value)?"{…}":value}function View(spec,options){options=options||{};vegaDataflow.Dataflow.call(this);options.loader&&this.loader(options.loader);options.logger&&this.logger(options.logger);null!=options.logLevel&&this.logLevel(options.logLevel);if(options.locale||spec.locale){var loc=
...
[];this._eventConfig=initializeEventConfig(spec.eventConfig);this.globalCursor(this._eventConfig.globalCursor);const ctx=runtime(this,spec,options.expr);this._runtime=ctx;this._signals=ctx.signals;this._bind=(spec.bindings||[]).map(_=>({state:null,param:vegaUtil.extend({},_)}));ctx.root&&ctx.root.set(loc);loc.source=ctx.data.root.input;this.pulse(ctx.data.root.input,this.changeset().insert(loc.items));this._width=this.width();this._height=this.height();this._viewWidth=viewWidth$jscomp$0(this,this._width);
...
handler){op$jscomp$0=(op$jscomp$0._targets||[]).filter(op=>op._update&&op._update.handler===handler);return op$jscomp$0.length?op$jscomp$0[0]:null}function addOperatorListener(view,name,op,handler){let h=findOperatorHandler(op,handler);h||(h=trap(view,()=>handler(name,op.value)),h.handler=handler,view.on(op,null,h));return view}function removeOperatorListener(view,op,handler){(handler=findOperatorHandler(op,handler))&&op._targets.remove(handler);return view}const NO_TRAP={trap:!1};var Skip={skip:!0};
vegaUtil.inherits(View,vegaDataflow.Dataflow,{async evaluate(encode,prerun,postrun){await vegaDataflow.Dataflow.prototype.evaluate.call(this,encode,prerun);if(this._redraw||this._resize)try{this._renderer&&(this._resize&&(this._resize=0,resizeRenderer(this)),await this._renderer.renderAsync(this._scenegraph.root)),this._redraw=!1}catch(e){this.error(e)}postrun&&vegaDataflow.asyncCallback(this,postrun);return this},dirty(item){this._redraw=!0;this._renderer&&this._renderer.dirty(item)},description(text){if(arguments.length){const desc=
null!=text?text+"":null;desc!==this._desc&&ariaLabel(this._el,this._desc=desc);return this}return this._desc},container(){return this._el},scenegraph(){return this._scenegraph},origin(){return this._origin.slice()},signal(name,value,options){const op=lookupSignal(this,name);return 1===arguments.length?op.value:this.update(op,value,options)},width(_){return arguments.length?this.signal("width",_):this.signal("width")},height(_){return arguments.length?this.signal("height",_):this.signal("height")},
padding(_){return arguments.length?this.signal("padding",padding$jscomp$0(_)):padding$jscomp$0(this.signal("padding"))},autosize(_){return arguments.length?this.signal("autosize",_):this.signal("autosize")},background(_){return arguments.length?this.signal("background",_):this.signal("background")},renderer(type){if(!arguments.length)return this._renderType;vegaScenegraph.renderModule(type)||vegaUtil.error("Unrecognized renderer type: "+type);type!==this._renderType&&(this._renderType=type,this._resetRenderer());
return this},tooltip(handler){if(!arguments.length)return this._tooltip;handler!==this._tooltip&&(this._tooltip=handler,this._resetRenderer());return this},loader(loader){if(!arguments.length)return this._loader;loader!==this._loader&&(vegaDataflow.Dataflow.prototype.loader.call(this,loader),this._resetRenderer());return this},resize(){this._autosize=1;return this.touch(lookupSignal(this,"autosize"))},_resetRenderer(){this._renderer&&(this._renderer=null,this.initialize(this._el,this._elBind))},_resizeView:function(viewWidth,
viewHeight,width,height,origin,auto){this.runAfter(view=>{let rerun=0;view._autosize=0;view.width()!==width&&(rerun=1,view.signal("width",width,Skip),view._resizeWidth.skip(!0));view.height()!==height&&(rerun=1,view.signal("height",height,Skip),view._resizeHeight.skip(!0));view._viewWidth!==viewWidth&&(view._resize=1,view._viewWidth=viewWidth);view._viewHeight!==viewHeight&&(view._resize=1,view._viewHeight=viewHeight);if(view._origin[0]!==origin[0]||view._origin[1]!==origin[1])view._resize=1,view._origin=
origin;rerun&&view.run("enter");auto&&view.runAfter(v=>v.resize())},!1,1)},addEventListener(type,handler,options){let callback=handler;options&&!1===options.trap||(callback=trap(this,handler),callback.raw=handler);this._handler.on(type,callback);return this},removeEventListener(type,handler){for(var handlers=this._handler.handlers(type),i=handlers.length,h,t;0<=--i;)if(t=handlers[i].type,h=handlers[i].handler,type===t&&(handler===h||handler===h.raw)){this._handler.off(t,h);break}return this},addResizeListener(handler){const l=
this._resizeListeners;0>l.indexOf(handler)&&l.push(handler);return this},removeResizeListener(handler){var l=this._resizeListeners;handler=l.indexOf(handler);0<=handler&&l.splice(handler,1);return this},addSignalListener(name,handler){return addOperatorListener(this,name,lookupSignal(this,name),handler)},removeSignalListener(name,handler){return removeOperatorListener(this,lookupSignal(this,name),handler)},addDataListener(name,handler){return addOperatorListener(this,name,dataref(this,name).values,
handler)},removeDataListener(name,handler){return removeOperatorListener(this,dataref(this,name).values,handler)},globalCursor(_){if(arguments.length){if(this._globalCursor!==!!_){const prev=setCursor(this,null);this._globalCursor=!!_;prev&&setCursor(this,prev)}return this}return this._globalCursor},preventDefault(_){return arguments.length?(this._preventDefault=_,this):this._preventDefault},timer:function(callback,delay){this._timers.push(d3Timer.interval(function(elapsed){callback({timestamp:Date.now(),
elapsed})},delay))},events:function(source,type,filter){var view=this,s=new vegaDataflow.EventStream(filter);filter=function(e,item){view.runAsync(null,()=>{var JSCompiler_temp;if(JSCompiler_temp="view"===source){var def=view._eventConfig.defaults;JSCompiler_temp=def.prevent;def=def.allow;JSCompiler_temp=!1===JSCompiler_temp||!0===def?!1:!0===JSCompiler_temp||!1===def?!0:JSCompiler_temp?JSCompiler_temp[type]:def?!def[type]:view.preventDefault()}JSCompiler_temp&&e.preventDefault();s.receive(eventExtend(view,
...
type);type=module.handler||vegaScenegraph.CanvasHandler;module=el?module.renderer:module.headless;view._renderer=module?initializeRenderer(view,view._renderer,el,module):null;view._handler=initializeHandler(view,view._handler,el,type);view._redraw=!0;el&&"none"!==config&&(elBind=elBind?view._elBind=lookup(view,elBind,!0):el.appendChild(element("form",{"class":"vega-bindings"})),view._bind.forEach(_=>{_.param.element&&"container"!==config&&(_.element=lookup(view,_.param.element,!!_.param.input))}),
view._bind.forEach(_=>{bind$jscomp$0(view,_.element||elBind,_)}));return view},toImageURL:async function(type,scaleFactor){type!==vegaScenegraph.RenderType.Canvas&&type!==vegaScenegraph.RenderType.SVG&&type!==vegaScenegraph.RenderType.PNG&&vegaUtil.error("Unrecognized image type: "+type);scaleFactor=await renderHeadless(this,type,scaleFactor);type===vegaScenegraph.RenderType.SVG?(type=scaleFactor.svg(),type=new Blob([type],{type:"image/svg+xml"}),type=window.URL.createObjectURL(type)):type=scaleFactor.canvas().toDataURL("image/png");
return type},toCanvas:async function(scaleFactor,opt){return(await renderHeadless(this,vegaScenegraph.RenderType.Canvas,scaleFactor,opt)).canvas()},toSVG:async function(scaleFactor){return(await renderHeadless(this,vegaScenegraph.RenderType.SVG,scaleFactor)).svg()},getState:function(options){return this._runtime.getState(options||{data:dataTest,signals:signalTest,recurse:!0})},setState:function(state){this.runAsync(null,v=>{v._trigger=!1;v._runtime.setState(state)},v=>{v._trigger=!0});return this}});
exports.View=View;Object.defineProperty(exports,"__esModule",{value:!0})})}
